...
export function ChatPanel({ onNavigatePatient }: ChatPanelProps) {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-brand-500 to-brand-700 text-white shadow-lg shadow-brand-500/30 transition-all hover:shadow-brand-500/40 hover:shadow-xl hover:scale-105 active:scale-95"
        title="Doctor Chat"
      >
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex h-[520px] w-[380px] flex-col overflow-hidden rounded-2xl border border-white/[0.06] bg-surface-1 shadow-2xl shadow-black/40">
      {/* Header */}
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            {msg.role === 'assistant' && (
              <div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-brand-500/20">
        ...
        {loading && (
          <div className="flex items-center gap-2">
            ...
            <div className="rounded-2xl rounded-bl-md bg-white/5 px-4 py-2">
      ...
      <div className="flex gap-1.5 px-3 pb-1">
        {[
          ...
          <button
                ...
                title={qa.label}
                className="flex items-center gap-1 rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[11px] text-slate-400 transition-colors hover:border-brand-500/40 hover:text-brand-400"
                aria-label={qa.label}
              >
      ...
      <div className="border-t border-white/[0.06] px-3 py-2">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200 placeholder:text-slate-500 focus:border-brand-500/50 focus:outline-none focus:ring-1 focus:ring-brand-500/30"
            disabled={loading}
          />
          <button
            type="button"
