  ...
  return (
    <div className="card">
      <div className="mb-3 flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-slate-700">Clinical Records</h3>
        {findings.length > 0 && (
      ...
      {!latest ? (
        <div className="flex h-40 flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 bg-slate-50/50">
          ...
          <p className="text-xs text-slate-400">No clinical records yet</p>
        </div>
      ) : (
        <div className="space-y-3">
          ...
          {segments.length > 0 && (
            <div className="max-h-96 overflow-y-auto rounded-lg border border-slate-100 bg-white">
              ...
              <div className="divide-y divide-slate-50">
                {segments.map((seg, i) => (
                  <button
                    ...
                    className={`flex w-full items-start gap-2 px-3 py-2 text-left transition hover:bg-emerald-50/50 ${
                      activeSegment === i ? 'bg-emerald-50 ring-1 ring-inset ring-emerald-200' : ''
                    }`}
                  ...
                  >
                    ...
                    <span
                      className={`text-xs leading-relaxed ${
                        activeSegment === i ? 'font-medium text-emerald-700' : 'text-slate-600'
                      }`}
                    >
                      {seg.text}
                    </span>
          ...
          {!!latest.details?.key_diagnoses && (
            <div className="rounded-lg bg-slate-50 p-3">
              <p className="mb-1 text-xs font-medium text-slate-500">Key Diagnoses</p>
